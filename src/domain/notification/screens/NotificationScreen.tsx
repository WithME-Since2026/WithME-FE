...
export function NotificationScreen() {
  ...
  const handleAccept = (notificationId: number) => readNotificationMutation.mutate(notificationId);
  const handleReject = (notificationId: number) => readNotificationMutation.mutate(notificationId);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.header}>
        ...
        <Pressable onPress={() => readAllNotificationsMutation.mutate()} hitSlop={8}>
          <Text style={styles.markAllReadLabel}>모두 읽음</Text>
        </Pressable>
      </View>
